...
function setscale(...newScale) {
  ...
  outlet(1, ["scale_degrees"].concat(scaleDegrees));
  outlet(1, ["scale"].concat(scale));
}
function setscaledegrees(...newScaleDegrees) {
  if ((0, import_isEqual.default)(scaleDegrees, newScaleDegrees))
    return;
  let last = 0;
  const scale2 = [];
  for (let degree of newScaleDegrees) {
    scale2.push(degree - last);
    last = degree;
  }
  post("SCALE:", scale2);
  setscale(...scale2);
...
function list(channel, note, velocity) {
  note = note - center;
  let step = note % scale.length;
  if (step < 0)
    step = scale.length + step;
  if (scale[step] === 0)
    return;
